if not status is-login; or inside-tmux
    # bin folders
    for path in '/usr/local/bin' '/usr/sbin' '/sbin' '/usr/bin'  '/usr/libexec' "$HOME/bin" "$HOME/.bin" "$HOME/.local/bin"
        if [ -d $path -a -r $path ]
    ...
    # ruby gem
    if test -e "$HOME/.gem/ruby"; and type -q ruby
